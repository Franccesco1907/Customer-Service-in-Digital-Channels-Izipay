...
import { SYSTEM_ASSISTANCE_CONTEXT } from './common/constants/prompts.context';
import { HttpService } from '@nestjs/axios';
import { catchError, firstValueFrom } from 'rxjs';
import { AxiosError } from 'axios';
import { IA_SERVICE, QUERIES_SERVICE, SOCIALNETWORK_SERVICE } from './common/constants';
...
`}]
  ...
  constructor(private readonly configService: ConfigService,
    ...
    @Inject(SOCIALNETWORK_SERVICE)
    private readonly socialNetworkService: ClientProxy,
    ) {
  ...
  async processFacebookMessageInput(input: FacebookMessageEntry) {
    const messagesInChat = (await this.chatBotRepository.find({socialNetworkUserId: input.entry[0].messaging[0].sender.id})).sort((a,b) => a.messageOrder - b.messageOrder);
    const context = [];
    if(messagesInChat.length === 0) {
      const initialContext = new CreateChatBotDto();
      initialContext.createdAt = new Date();
      initialContext.hasQuestionIntention = null;
      initialContext.feelings = null;
      initialContext.message = SYSTEM_ASSISTANCE_CONTEXT;
      initialContext.messageOrder = 0;
      initialContext.messageSendAt = Date.now();//input.entry[0].messaging[0].timestamp;
      initialContext.role = "system";
      initialContext.socialNetworkUserId = input.entry[0].messaging[0].sender.id;
      await this.chatBotRepository.create(initialContext);
      context.push({role: initialContext.role, content: initialContext.message});
    } else {
      for (const messageInChat of messagesInChat) {
        context.push({role: messageInChat.role, content: messageInChat.message});
      }
    }

    console.log(context);

    const newUserInput = new CreateChatBotDto();
    newUserInput.createdAt = new Date();
    newUserInput.hasQuestionIntention = null;
    newUserInput.feelings = null;
    newUserInput.message = input.entry[0].messaging[0].message.text;
    newUserInput.messageOrder = context.length;
    newUserInput.messageSendAt = input.entry[0].messaging[0].timestamp;
    newUserInput.role = "user";
    newUserInput.socialNetworkUserId = input.entry[0].messaging[0].sender.id;
    await this.chatBotRepository.create(newUserInput);    
    
    const chatbotResponse = await this.collectMessages(context, newUserInput.message)

    const newChatBotResponse = new CreateChatBotDto();
    newChatBotResponse.createdAt = new Date();
    newChatBotResponse.hasQuestionIntention = null;
    newChatBotResponse.feelings = null;
    newChatBotResponse.message = chatbotResponse
    newChatBotResponse.messageOrder = context.length - 1;
    newChatBotResponse.messageSendAt = Date.now();
    newChatBotResponse.role = "assistant";
    newChatBotResponse.socialNetworkUserId = input.entry[0].messaging[0].sender.id;
    await this.chatBotRepository.create(newChatBotResponse);     
    await this.postData('http://127.0.0.1:8083/social-networks/message-fb/answer', {
      token: "EAAOKcO7VHykBO2S2bPYYQHAWKXnLG7bqPOoi8AsfJs7A9aOgZAYK2cZCGls7tZCHBMoI7x7FUcKVkZCcTpIMaAMzTh258ZC9QpvWlkPIsbyK6zho8OFYZCVR0fipfOdwU3OBHK0SQQiVgGotKxZAPmvmiTgprkyG8bltr3rZCyyrphAw0I10ZANldIunCRbRztOQZD",
      userSocialNetworkId: newUserInput.socialNetworkUserId,
